<?php
    ...
    if(isset($_GET['update'])){
        if(!empty($_GET['updcol2'])){
            ...
            if(isset($_GET["updprimKey"])){ //if we selected primary key
                $updprimKey = $_GET["updprimKey"];
        ...
        else{ //if we didnt select primary key
            foreach ($column as $colkey => $value){
                $updsql = "{$column[$colkey]} = '{$upd[$colkey]}'";
    ...
    echo "<tr>";
    if(isset($_GET["updprimKey"])) //if we select prim key
    ...
    if(isset($_GET['update'])){
        ...
        $result = mysqli_query($conn, $sql);
			...
			while($row = mysqli_fetch_assoc($result)){
                ...
                <input type="hidden" name="name" value='.$row["Country_Name"].'> 
