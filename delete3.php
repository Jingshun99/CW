<?php
    ...
    echo "<tr>";
        ...
        if(!empty($_GET["col3"])){
            foreach($_GET["col3"] as $col3){
                if($col3 == 'Country_abb'){
                    echo "$col3<input type = 'hidden', value = '$key[0]', name = 'primKey[]' checked> : <input type = 'text' name = 'updprimKey1' value = '$key[0]' id='datacol' class='data'>";
                    echo "<input type = 'hidden', value = '$key[1]', name = 'primKey[]'>";
                }
                else if($col3 == 'Language'){
                    echo "<input type = 'hidden', value = '$key[0]', name = 'primKey[]'>";
                    echo "$col3<input type = 'hidden', value = '$key[1]', name = 'primKey[]' checked> : <input type = 'text' name = 'updprimKey2' value = '$key[1]' id='datacol' class='data'>";
                }
                else{
                    echo "<input type = 'hidden', value = '$key[0]', name = 'primKey[]'>";
                    echo "<input type = 'hidden', value = '$key[1]', name = 'primKey[]'>";
